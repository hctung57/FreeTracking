...
function waitForDocumentReady(timeoutMs = 3e4) {
...
function hasMeaningfulContent(pageText) {
  ...
  return text.includes("tracking") || text.includes("status") || text.includes("delivered") || text.includes("in transit") || text.includes("usps");
}
function waitForMeaningfulContent(timeoutMs = 3e4) {
  return new Promise((resolve, reject) => {
...
function getLatestStepElement(root = document) {
  ...
  return steps.find(
    (element) => !element.classList.contains("current-step") && !element.classList.contains("toggle-history-container")
  ) || null;
}
...
function readBatchTrackingStatus(trackingNumbers) {
  ...
  const byTrackingNumber = /* @__PURE__ */ new Map();
  ...
  return {
    ...
    items: trackingNumbers.map((trackingNumber) => ({
      ...
      error: byTrackingNumber.get(trackingNumber)?.error || (!byTrackingNumber.has(trackingNumber) ? "Tracking number was not found in USPS batch response" : "")
    }))
  };
}
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  handleRequest().then((response) => sendResponse(response)).catch((error) => sendResponse({ ok: false, error: error instanceof Error ? error.message : String(error) }));
  return true;
});
//# sourceMappingURL=usps-content.js.map
