...
function createWorkbook(results) {
  const workbook = utils.book_new();
  const rows = buildWorkbookRows(results);
  const sheet = utils.json_to_sheet(rows, {
    header: ["trackingNumber", "tracking_detail", "error"]
  });
  utils.book_append_sheet(workbook, sheet, "USPS Tracking");
  return workbook;
}
function createWorkbookFromRows(rows, headers, sheetName = "Orders") {
  const workbook = utils.book_new();
...
function workbookToBlob(workbook, format = "xlsx") {
  const normalizedFormat = format === "csv" ? "csv" : "xlsx";
  const arrayBuffer = writeSync(workbook, { bookType: normalizedFormat, type: "array" });
  const mimeType = normalizedFormat === "csv" ? "text/csv;charset=utf-8" : "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
...
function normalizeTrackingInput(rawText) {
  const seen = /* @__PURE__ */ new Set();
  return rawText.split(/\r?\n|,|\s+/).map((value) => value.trim()).filter(Boolean).filter((value) => {
    if (seen.has(value)) {
      return false;
    }
    seen.add(value);
...
// src/app/app.js
var TRACKING_ID_COLUMN = "TRACKING_ID";
var TRACKING_STATUS_COLUMN = "TRACKING_STATUS";
var trackingInput = document.getElementById("trackingInput");
var fileInput = document.getElementById("fileInput");
var fileInfo = document.getElementById("fileInfo");
var manualModeButton = document.getElementById("manualModeButton");
...
var jobStatePill = document.getElementById("jobStatePill");
...
async function handleFileUpload(file) {
  ...
  const workbook = readSync(fileBuffer, { type: "array" });
